...
if ( ! class_exists( 'BRANDING_Admin_Master', false ) ) :
/**
 * BRANDING_Admin_Master Class.
 */
class BRANDING_Admin_Master {
    ...
    public function __construct() {
        ...
        switch ($this->page) {
            case 'BRANDING-Options-pop-ups':
                $this->setup_default_ui();
                break;

            case 'BRANDING-Options-triggers':
                $this->setup_triggers_ui();
                break;

            case 'BRANDING-Options-extentions':
                $this->setup_extensions_ui();
                break;
            case 'BRANDING-Options-settings':
    ...
    private function setup_default_ui() {
        ...
        if(has_filter('branding_add_tab_create_pop_ups')) {
            $this->tabs = apply_filters('branding_add_tab_create_pop_ups', $this->tabs);
        }
    ...
    private function setup_triggers_ui() {
        $this->tabs = array(
            ...
            'edittriggers'   => __("Edit & Delete Triggers", 'plugin-textdomain'),
        );

        if(has_filter('branding_add_tab_triggers')) {
            $this->tabs = apply_filters('branding_add_tab_triggers', $this->tabs);
        }
    ...
    private function setup_dynamic_ui() {
        ...
        // A filter to add tabs to the submenu page. So the submenu extensions can have their own separate plugins that add tabs to it. The name of this filter will be 'branding_add_tab_' plus the one-word unique identifer for this extension, i.e., the word that is displayed in the BRANDING plugin main menu.  
        if(has_filter('branding_add_tab_'.$page[2])) {
            $this->tabs = apply_filters('branding_add_tab_'.$page[2], $this->tabs);
        }
    ...
    private function output_indiv_tab() {
        ...
        if(!file_exists(BRANDING_CLASSES_UI_ADMIN_DIR.$filename)){
            $path = $this->build_extension_path();
            if(is_dir($path)){
           ...
           require_once(BRANDING_CLASSES_UI_ADMIN_DIR.$filename);
        }
    }
    ...
    private function build_extension_path() {
        ...
        foreach($this->active_plugins as $plugin){
            if(strpos($plugin, 'branding-') !== false){
                if(strpos($plugin, $this->activetab) !== false){
                    ...
                    if($temp[2] === $this->activetab.'.php'){
                        ...
                        $path = BRANDING_ROOT_WP_PLUGINS_DIR.$temp[0].'-'.$this->activetab.'/'.$filename;
                    } else {
                ...
                if(file_exists($path) && !is_dir($path)){
                    ...
                    if(strpos($plugin, $page[2]) !== false){
                        $path = BRANDING_ROOT_WP_PLUGINS_DIR.'branding-'.$page[2];
                        if(file_exists($path)){
                            return $path;
                        }
                    }
